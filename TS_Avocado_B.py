...
#------------------------------------------------------------------------------
# Setting the true MNL model
# only the discount price combiantion related to 68% market shares of avocado
...
def TS_Beta(T,price_org,discounts,b_samples):
    ...
    while t<=T:
        ...
        while True:
            ...
            #-------------------------------------------------------------------
            # record the price combination be selected at time period t
            price_comb_selected.append(optimal_index_ts)
...
b_revenue_filename = "b_revenue_t_{}_{}.csv".format(T, "_".join(map(str, np.round(b_samples,2))))
...
b_regret_filename = "b_regret_t_{}_{}.csv".format(T, "_".join(map(str, np.round(b_samples,2))))
